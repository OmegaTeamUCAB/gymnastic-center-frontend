...
class LessonBloc extends Bloc<LessonEvent, LessonState> {
  ...
  Future<void> _changeToFirstLesson(ChangeToFirstLesson event, Emitter<LessonState> emit) async {
    ...
    emit(state.copyWith(firstLesson: state.courseLessons.first.id == lesson.id, lastLesson: state.courseLessons.last.id == lesson.id, lesson: lesson, lessonStatus: LessonStatus.loaded));
  }
    ...
    emit(state.copyWith(lessonStatus: LessonStatus.loaded , courseLessons: event.lessons, firstLesson: lessons.first.id == lessons.first.id, lastLesson: lessons.first.id == lessons.last.id, lesson: lessons.first, courseImage: event.courseImage, courseId: event.course));
  }
  ...
  LessonProgress? getCurrentLessonProgress() {
    ...
    return state.progress.lessonProgress.where((element) => element.lessonId == state.lesson.id).toList()[0];
  }
